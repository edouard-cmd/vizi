...
// ============================================================
// VISIMER - PATCH JS MOBILE BOTTOM SHEET v1
// Bottom sheet 3 hauteurs (peek/mid/full) pour #spotDrawer mobile.
// ============================================================
...
(function() {
  ...
  function getTranslateForSnap(snap) {
    ...
    return window.innerHeight - (SNAP_HEIGHTS[snap] * vh());
  }

  function applySnapState(snap) {
    var drawer = sheetState.drawer;
    if (!drawer) return;
    sheetState.currentSnap = snap;
    drawer.classList.remove('sheet-peek', 'sheet-mid', 'sheet-full', 'sheet-dragging');
    drawer.classList.add('sheet-' + snap);
    document.body.classList.remove('vz-drawer-peek', 'vz-drawer-mid', 'vz-drawer-full');
    document.body.classList.add('vz-drawer-' + snap);
    drawer.style.transform = '';
  }

  function findClosestSnap(translate, vel) {
    if (Math.abs(vel) > DRAG_THRESHOLD_VELOCITY) {
      var current = sheetState.currentSnap;
      if (vel > 0) {
    ...
    var positions = {
      peek: getTranslateForSnap('peek'),
      mid: getTranslateForSnap('mid'),
      full: getTranslateForSnap('full')
    };
    var closest = 'mid';
    var minDist = Infinity;
  ...
  function onPointerDown(e) {
    ...
    sheetState.startY = e.touches ? e.touches[0].clientY : e.clientY;
    sheetState.lastY = sheetState.startY;
    sheetState.lastTime = Date.now();
    sheetState.velocity = 0;
    sheetState.startTranslate = getTranslateForSnap(sheetState.currentSnap);
  ...
  function onPointerMove(e) {
    ...
    var now = Date.now();
    var dt = now - sheetState.lastTime;
    if (dt > 0) {
      sheetState.velocity = (y - sheetState.lastY) / dt;
    }
    sheetState.lastY = y;
    sheetState.lastTime = now;
    var delta = y - sheetState.startY;
    var newTranslate = sheetState.startTranslate + delta;
    var minTranslate = getTranslateForSnap('full');
    var maxTranslate = window.innerHeight + 100;
    newTranslate = Math.max(minTranslate, Math.min(maxTranslate, newTranslate));
    sheetState.currentTranslate = newTranslate;
    sheetState.drawer.style.transform = 'translateY(' + newTranslate + 'px)';
    e.preventDefault();
  }
  ...
  function onPointerUp() {
    ...
    var drawer = sheetState.drawer;
    if (!drawer) return;
    var peekPos = getTranslateForSnap('peek');
    if (sheetState.currentTranslate > peekPos + (vh() * CLOSE_THRESHOLD_VH) && sheetState.velocity > 0.3) {
      if (typeof closeSpotPopup === 'function') {
        closeSpotPopup();
      }
      document.body.classList.remove('vz-drawer-peek', 'vz-drawer-mid', 'vz-drawer-full');
      return;
    }
    var targetSnap = findClosestSnap(sheetState.currentTranslate, sheetState.velocity);
    applySnapState(targetSnap);
  }
  ...
  function setupDrawerObserver() {
    ...
    if (sheetState.handle) {
      sheetState.handle.addEventListener('touchstart', onPointerDown, { passive: false });
      sheetState.handle.addEventListener('touchmove', onPointerMove, { passive: false });
      sheetState.handle.addEventListener('touchend', onPointerUp);
      sheetState.handle.addEventListener('touchcancel', onPointerUp);
      sheetState.handle.addEventListener('mousedown', onPointerDown);
      document.addEventListener('mousemove', onPointerMove);
      document.addEventListener('mouseup', onPointerUp);

      var tapStartY = 0;
      var tapStartTime = 0;
      sheetState.handle.addEventListener('touchstart', function(e) {
        tapStartY = e.touches[0].clientY;
        tapStartTime = Date.now();
      }, { passive: true });
      sheetState.handle.addEventListener('touchend', function(e) {
        if (!isMobileViewport()) return;
        var endY = e.changedTouches[0].clientY;
        var dy = Math.abs(endY - tapStartY);
        var dt = Date.now() - tapStartTime;
        if (dy < 8 && dt < 250) {
          var current = sheetState.currentSnap;
          var next = current === 'peek' ? 'mid' : (current === 'mid' ? 'full' : 'peek');
          applySnapState(next);
        }
      }, { passive: true });
    }
  ...
  function init() {
    setupDrawerObserver();
    console.log('[VISIMER] Bottom sheet mobile v1 initialisé');
  }
