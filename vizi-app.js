...
var WEBCAMS = [
  ...
  { id:'wc_bernieres', name:'Bernieres-sur-Mer - Club de Voile', lat:49.3352, lon:-0.4247, url:'https://www.youtube-nocookie.com/embed/tox9v6-v75k', pageUrl:'https://www.cvbernieres.fr/webcam.php' },
  // Courseulles : webcam Viewsurf des Ports du Calvados, camera rotative,
  // differe 15 min. Player court pv.viewsurf (le lecteur Joada de la page
  // publique refuse l'embed externe en 403).
  { id:'wc_courseulles_capit', name:'Courseulles - Capitainerie', lat:49.3350, lon:-0.4632, url:'https://pv.viewsurf.com/2460/Port-de-Courseulles-sur-mer-Capitainerie', pageUrl:'https://viewsurf.com/univers/plage/vue/19412-france-basse-normandie-courseulles-sur-mer-port-de-courseulles-sur-mer-capitainerie' },
  // Cotentin (Manche)
...
/* ============================================================
   ...
   ------------------------------------------------------------
   Deux etats seulement, tranches ici et nulle part ailleurs :
     1. rien de memorise -> France entiere, l'utilisateur zoome ou il veut
     2. vue memorisee    -> setView sec, sans animation
   Aucune interface, aucune question, aucun texte a lire avant de voir la
   carte : la memorisation ne se voit pas, elle se constate a la deuxieme
   ouverture. Aucune autre partie du code n'a le droit de fixer centre ou
   zoom au demarrage. Le lien profond ?p=lat,lon est la seule exception :
   il court-circuite VZ_VIEW et ne memorise rien (le point partage par un
   tiers n'est pas la vue d'accueil du destinataire).
   Restauration en setView sans animation : un vol France -> cote chargerait
   les tuiles de tous les niveaux intermediaires, inacceptable en 4G.
   ============================================================ */
    ...
    persistTimer = setTimeout(function() {
      ...
      try {
        localStorage.setItem(KEY, JSON.stringify({
          lat: +c.lat.toFixed(5), lon: +c.lng.toFixed(5), z: S.map.getZoom()
        }));
      } catch (e) {}
    }, 600);
  }
...
function vzWindEnsureCtrl_() {
  ...
  if (!document.getElementById('vzWindCtrlStyle')) {
    ...
    +   "#vzSheet.sheet-half ~ #vzWindCtrl,#vzSheet.sheet-full ~ #vzWindCtrl{display:none !important;}"
    + "}";
    (document.head || document.documentElement).appendChild(st);
  }
...
(function () {
  ...
  function css() {
    ...
    + "#vzSearch .vz-search-glass{display:flex;flex-shrink:0;color:#0A1520;}"
    + "#vzSearch .vz-search-glass svg{width:18px;height:18px;}"
    + "#vzSearch .vz-search-in{flex:1;min-width:0;background:none;border:none;outline:none;color:#0A1520;font-family:'Inter',sans-serif;font-size:13.5px;font-weight:500;}"
    + "#vzSearch .vz-search-in::placeholder{color:#5F6B73;font-weight:400;}"
    + "#vzSearch .vz-search-clear{display:none;align-items:center;justify-content:center;width:26px;height:26px;flex-shrink:0;padding:0;background:none;border:none;border-radius:50%;color:#0A1520;cursor:pointer;}"
    + "#vzSearch .vz-search-clear svg{width:14px;height:14px;}"
    + "#vzSearch .vz-search-clear:hover{color:#0A1520;background:rgba(10,21,32,0.08);}"
    + "#vzSearch.has-val .vz-search-clear{display:flex;}"
    + "#vzSearch .vz-search-list{display:none;margin-top:8px;padding:5px;max-height:min(46vh,330px);overflow-y:auto;background:#FFFFFF;border:2px solid #0A1520;border-radius:14px;box-shadow:0 6px 20px rgba(8,17,27,0.3);}"
    + "#vzSearch .vz-search-list.show{display:block;}"
    + "#vzSearch .vz-search-item{display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:9px;cursor:pointer;}"
    + "#vzSearch .vz-search-item:hover,#vzSearch .vz-search-item.sel{background:rgba(10,21,32,0.07);}"
    + "#vzSearch .vz-search-ico{display:flex;flex-shrink:0;color:#1A6B5D;opacity:1;}"
    ...
    + "#vzSearch .vz-search-name{font-size:13.5px;font-weight:600;color:#0A1520;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
    + "#vzSearch .vz-search-sub{font-family:'IBM Plex Mono',monospace;font-size:10.5px;color:#5F6B73;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
